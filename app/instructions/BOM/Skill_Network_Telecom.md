...
The exported BOM must be self-explanatory. A procurement reviewer must be able
to understand every quantity **without reading the conversation**. Every line
item must include these five fields:
...
- Dependent lines (e.g. license per router, SmartNet per device) use the parent
  count as the driver
