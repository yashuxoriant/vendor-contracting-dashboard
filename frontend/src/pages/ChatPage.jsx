...
import {
  ...
  InsertDriveFile, TableChart, PictureAsPdf, Article, KeyboardArrowDown,
...
function backendBOMtoFrontend(backendBOM, projectName) {
  ...
  return {
    ...
    optionalRecommendations: (backendBOM.optional_recommendations || []).map((r, i) => ({
      ...
      qty:                 r.qty || 1,
      vendor:              r.vendor || '',
      estimatedUnitPrice:  r.estimated_unit_price || 0,
      recommendationReason: r.recommendation_reason || '',
      recommendationType:  r.recommendation_type || 'unconfirmed_assumption',
    })),
...
async function exportBOMExcel(bom, dispatchFn) {
  try {
    ...
    const backendPayload = {
      ...
      optional_recommendations: (bom.optionalRecommendations || []).map(r => ({
        description:           r.description,
        category:              r.category,
        sku:                   r.sku || '',
        qty:                   r.qty,
...
// Reusable helper: maps raw backend agent_state keys → INTAKE_STEPS display keys.
// Call this both on streaming onDone and when restoring a saved session so the
// intake progress checkboxes are always in sync with what the backend knows.
function _mapAgentStateToIntakeFields(st) {
  if (!st) return {}
  return {
    ...st,
    site_entity_scope:               st.site_entity_scope               || st.site_count                   || null,
    site_classification_size:        st.site_classification_size        || st.user_count || st.site_size   || null,
    vendor_standard_preferred:       st.vendor_standard_preferred       || st.vendor_standard              || null,
    site_classification_criticality: st.site_classification_criticality || st.site_criticality             || null,
    hardware_layers_scope: (Array.isArray(st.requested_layers) && st.requested_layers.length > 0)
      ? st.requested_layers.join(', ')
      : (st.hardware_layers_scope || null),
  }
}
...
const INTAKE_STEPS = [
  ...
  { key: 'hardware_layers_scope',           label: 'Hardware Layers' },
]
...
export default function ChatPage() {
  ...
  const [intakeAccordionOpen, setIntakeAccordionOpen] = useState(true)  // intake progress accordion
        ...
        if (savedSid) {
          ...
          chatApi.getTranscript(savedSid, 'demo_user')
            .then(transcript => {
              ...
              // Restore intake progress checkboxes from saved agent_state
              if (transcript?.agent_state && Object.keys(transcript.agent_state).length) {
                setIntakeFields(_mapAgentStateToIntakeFields(transcript.agent_state))
              }
            })
            .catch(() => {
  ...
  const switchToSession = (sessionItem, linkedBom = null) => {
    ...
    chatApi.getTranscript(sid).then(data => {
      ...
      if (data.bom) {
        // Backend returns the raw Cosmos BOM doc — normalise to frontend shape
        const b = data.bom
        const normBom = {
          ...b,
          id:          b.id || b.bom_id,
          name:        b.name || b.bom_name || `${b.project || 'BOM'} Draft`,
          lineItems:   b.lineItems || b.line_items || [],
          totalValue:  b.totalValue ?? b.total_value ?? b.totals?.total_otc ?? 0,
          status:      b.status || 'draft',
          version:     b.version || b.revision || 1,
        }
        setLocalBOM(normBom)
        dispatch(setCurrentBOM(normBom))
        saveSessionBOM(sid, normBom)   // cache so offline restore works next time
      } else {
        // Transcript endpoint has no BOM — restore from localStorage cache.
        const cachedBOM = loadSessionBOM(sid)
        if (cachedBOM) { setLocalBOM(cachedBOM); dispatch(setCurrentBOM(cachedBOM)) }
      }
      // Restore intake progress checkboxes from saved agent_state
      if (data.agent_state && Object.keys(data.agent_state).length) {
        setIntakeFields(_mapAgentStateToIntakeFields(data.agent_state))
      }
    }).catch(() => {
        ...
        await chatApi.sendMessageStream(
          ...
          (finalEvt) => {
            ...
            if (finalEvt.agent_state && Object.keys(finalEvt.agent_state).length) {
              // Backend sends authoritative agent_state — map keys and update checkboxes.
              const mapped = _mapAgentStateToIntakeFields(finalEvt.agent_state)
              setIntakeFields(prev => ({ ...prev, ...mapped }))
              ...
              // Expand date matching: all months + quarters + year-end
              if (!updates.required_by_date           && /january|february|april|may|june|july|august|september|october|november|december|q[1-4]\s*20\d{2}|year.?end/i.test(txt)) updates.required_by_date = true
              if (!updates.required_by_date           && /december|january|february|march|q[1-4]\s*20\d\d/i.test(txt))               updates.required_by_date = true
              // M&A Phase — skill assumes Day-1; tick as soon as category is known
              if (!updates.ma_phase                   && (ctx.category || updates.workstream_category))                               updates.ma_phase = 'Day-1'
              // Category — populate from session ctx when present
              if (!updates.workstream_category        && ctx.category)                                                                updates.workstream_category = ctx.category
              // Conveyance — tick when user provides conveyance intent
              if (!updates.conveyance_status          && /\bconvey|\bnot convey|\bdedicated\b|\bshared\b|\bgreenfield\b|\bnew build\b|\bnot transfer/i.test(txt)) updates.conveyance_status = true
              // Users / Scale — qualitative size phrases + numeric user counts
              if (!updates.site_classification_size   && /\bsmall\b|\bmedium\b|\blarge\b|\bfull.?floor\b|\bff\b|\bsame size\b|\d+\s*user|\d+\s*staff/i.test(txt)) updates.site_classification_size = true
              // Hardware layers — any layer keyword
              if (!updates.hardware_layers_scope      && /sd.?wan|firewall|lan switch|access point|\bwlan\b|\bwifi\b|\bwireless\b/i.test(txt))                   updates.hardware_layers_scope = true
        ...
        {(() => {
          ...
          return (
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              {/* ── Intake Progress Accordion ── */}
              {hasProgress && (
                <Box sx={{ flexShrink: 0, borderBottom: '1px solid #F3F4F6' }}>
                  <Box
                    onClick={() => setIntakeAccordionOpen(o => !o)}
                    sx={{ px: 1.5, py: 0.75, cursor: 'pointer', display: 'flex', alignItems: 'center',
                      justifyContent: 'space-between', bgcolor: '#FAFAFA', userSelect: 'none',
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                      ...
                      <Chip
                        label={displayPct + '%'}
                        size="small"
                        sx={{ fontSize: '0.55rem', height: 14, fontWeight: 700,
                          bgcolor: displayPct === 100 ? '#D1FAE5' : '#FEF3C7',
                          color: displayPct === 100 ? '#065F46' : '#D04A02' }}
                      />
                    </Box>
                    <KeyboardArrowDown sx={{ fontSize: 14, color: '#9CA3AF',
                      transition: 'transform 0.2s',
                      transform: intakeAccordionOpen ? 'rotate(180deg)' : 'rotate(0deg)' }} />
                  </Box>

                  {intakeAccordionOpen && (
                    <Box sx={{ px: 1.5, pb: 1, maxHeight: 220, overflowY: 'auto' }}>
                      <Box sx={{ mb: 1, mt: 0.5 }}>
                        <LinearProgress variant="determinate" value={displayPct}
                          sx={{ height: 5, borderRadius: 3, bgcolor: '#FEF3C7',
                            '& .MuiLinearProgress-bar': { bgcolor: displayPct === 100 ? '#10B981' : '#D04A02', borderRadius: 3 } }} />
                        <Typography sx={{ fontSize: '0.6rem', color: '#9CA3AF', mt: 0.4 }}>
                      ...
                      {INTAKE_STEPS.map(step => {
                        ...
                        return (
                          <Box key={step.key} sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 0.5,
                            borderBottom: '1px solid #F9FAFB' }}>
                            <Box sx={{ width: 14, height: 14, borderRadius: '50%', flexShrink: 0,
                              bgcolor: done ? '#D1FAE5' : '#F3F4F6',
                              display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              <Typography sx={{ fontSize: '0.55rem', color: done ? '#10B981' : '#9CA3AF', fontWeight: 700, lineHeight: 1 }}>
                                {done ? '✓' : '·'}
                              </Typography>
                            </Box>
                            <Typography sx={{ fontSize: '0.67rem', color: done ? '#065F46' : '#6B7280',
              ...
              {/* ── BOM content (shown once generated) ── */}
              {currentBOM && (
                <>
            ...
            <Box sx={{ flex: 1, overflowY: 'auto', px: 1.25, py: 0.75 }}>
              ...
              {/* ── Optional Recommendations (assumptions pending confirmation) ── */}
              {false && (currentBOM.optionalRecommendations || []).length > 0 && (
                <Box sx={{ mt: 1, border: '1px solid #FDE68A', borderRadius: 1, overflow: 'hidden' }}>
                  <Box sx={{ px: 1.25, py: 0.6, bgcolor: '#FFFBEB', borderBottom: '1px solid #FDE68A', display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    <Typography sx={{ fontSize: '0.62rem', fontWeight: 700, color: '#92400E' }}>⚠ Assumed Items — Confirm Before Ordering</Typography>
                    <Chip label={currentBOM.optionalRecommendations.length} size="small" sx={{ fontSize: '0.55rem', height: 14, bgcolor: '#FEF3C7', color: '#92400E', ml: 'auto' }} />
                  </Box>
                  {currentBOM.optionalRecommendations.map(rec => (
                    <Box key={rec.id} sx={{ px: 1.25, py: 0.75, borderBottom: '1px solid #FEF9C3', bgcolor: '#FFFDF0' }}>
                      <Typography sx={{ fontSize: '0.65rem', fontWeight: 600, color: '#1F2937', lineHeight: 1.3 }}>{rec.description}</Typography>
                      <Typography sx={{ fontSize: '0.58rem', color: '#6B7280', mt: 0.25, lineHeight: 1.3 }}>{rec.recommendationReason}</Typography>
                      {rec.estimatedUnitPrice > 0 && (
                        <Typography sx={{ fontSize: '0.58rem', color: '#92400E', mt: 0.2 }}>
                          Est. {fmt(rec.estimatedUnitPrice)} × {rec.qty}
                        </Typography>
                      )}
                      <Box sx={{ display: 'flex', gap: 0.5, mt: 0.6 }}>
                        <Button size="small" variant="contained"
                          onClick={() => {
                            const newItem = {
                              id: 'li_conf_' + Date.now(),
                              lineNo: currentBOM.lineItems.length + 1,
                              category: rec.category,
                              description: rec.description,
                              unit: '/unit',
                              qty: rec.qty || 1,
                              unitPrice: rec.estimatedUnitPrice || 0,
                              extPrice: (rec.estimatedUnitPrice || 0) * (rec.qty || 1),
                              vendor: rec.vendor || '',
                              status: 'draft',
                              sku: rec.sku || '',
                              term: 'one-time',
                              qtyStatus: 'confirmed',
                              notes: 'Confirmed from Optional Recommendations',
                            }
                            const newItems = [...currentBOM.lineItems, newItem]
                            const newRecs = currentBOM.optionalRecommendations.filter(r => r.id !== rec.id)
                            dispatch(saveBOM({
                              ...currentBOM,
                              lineItems: newItems,
                              optionalRecommendations: newRecs,
                              totalValue: calculateBOMTotal(newItems),
                              updatedAt: new Date().toISOString(),
                            }))
                          }}
                          sx={{ fontSize: '0.58rem', py: 0.2, px: 0.75, textTransform: 'none', fontWeight: 700, bgcolor: '#059669', '&:hover': { bgcolor: '#047857' }, minWidth: 0 }}>
                          ✓ Add
                        </Button>
                        <Button size="small" variant="outlined"
                          onClick={() => {
                            const newRecs = currentBOM.optionalRecommendations.filter(r => r.id !== rec.id)
                            dispatch(saveBOM({ ...currentBOM, optionalRecommendations: newRecs, updatedAt: new Date().toISOString() }))
                          }}
                          sx={{ fontSize: '0.58rem', py: 0.2, px: 0.75, textTransform: 'none', fontWeight: 600, borderColor: '#D1D5DB', color: '#6B7280', minWidth: 0 }}>
                          ✕ Dismiss
                        </Button>
                      </Box>
                    </Box>
