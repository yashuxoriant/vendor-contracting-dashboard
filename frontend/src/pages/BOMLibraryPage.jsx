...
import {
  ...
  CloudUpload, DeleteOutline,
} from '@mui/icons-material'
import { setActiveBOMForRFQ, setCurrentBOM, archiveBOM, saveBOM, deleteBOM, resetToSeed } from '../store/slices/bomSlice'
...
export default function BOMLibraryPage() {
  ...
  const deletedIds = useSelector(s => s.bom.deletedIds || [])
  ...
  const [confirmDelete, setConfirmDelete] = useState(null)  // BOM object pending deletion
  ...
  // Load BOMs from backend on mount — merge into Redux, skipping tombstoned IDs
  const loadFromBackend = useCallback(async () => {
    ...
    try {
      ...
      // Read tombstone list directly from localStorage as a fallback in case
      // the Redux selector hasn't updated yet at the time this callback runs.
      const tombstones = (() => { try { return JSON.parse(localStorage.getItem('bom_deleted_ids') || '[]') } catch { return [] } })()
      boms
        .filter(b => !tombstones.includes(b.id))
        .filter(b => {
          // Name-based dedup: if a BOM with the same name already exists in the
          // list (e.g. loaded from the frontend Redux seed or a prior AI session),
          // skip adding a second copy with a different backend-assigned ID.
          const existing = bomList.find(x => x.name === b.name)
          return !existing
        })
        .forEach(b => dispatch(saveBOM(b)))
    } catch (err) {
    ...
    <Box sx={{ bgcolor: '#FAFAFA', p: 2 }}>
          ...
          <Paper key={bom.id} sx={{ mb: 1.25, border: isComparing ? '2px solid #3B82F6' : '1px solid #E5E7EB', overflow: 'hidden' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, p: 1.5, borderLeft: '5px solid ' + catColor }}>
              ...
              <Box sx={{ display: 'flex', gap: 0.25, flexShrink: 0, flexWrap: 'wrap' }}>
                ...
                <Tooltip title="Delete BOM"><IconButton size="small" onClick={() => setConfirmDelete(bom)} sx={{ color: '#EF4444' }}><DeleteOutline sx={{ fontSize: 16 }} /></IconButton></Tooltip>
      ...
      <Dialog open={!!confirmDelete} onClose={() => setConfirmDelete(null)} maxWidth="xs" fullWidth>
        <DialogTitle sx={{ pb: 0.5, display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography sx={{ fontWeight: 700, fontSize: '0.95rem' }}>Delete BOM</Typography>
        </DialogTitle>
        <DialogContent sx={{ pt: 1 }}>
          ...
          <Typography sx={{ fontSize: '0.74rem', color: '#6B7280', mt: 0.75 }}>
            This will remove it from the BOM Library and cannot be undone.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 2, pb: 1.5 }}>
          ...
          <Button size="small" variant="contained"
            onClick={() => {
              dispatch(deleteBOM(confirmDelete.id))
              bomService.delete(confirmDelete.id)  // best-effort backend delete
