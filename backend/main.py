...
async def startup_event():
    ...
    # Start SharePoint delta polling scheduler (no-op if SHAREPOINT_POLL_INTERVAL_MINUTES=0)
    try:
        from services.bom_delta_pipeline import start_scheduler
        start_scheduler()
    except Exception as e:
        logger.warning(f"BOM delta scheduler not started: {e}")
...
async def shutdown_event():
    ...
    try:
        from services.bom_delta_pipeline import stop_scheduler
        stop_scheduler()
    except Exception:
        pass
...
>>>>>>> 903d78a8132985b726ce5c6f3e6fc04daca13dcd
