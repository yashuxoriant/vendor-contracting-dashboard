...
async def stream_message(request: ChatMessageRequest, background_tasks: BackgroundTasks):
    ...
    async def event_stream():
        ...
        # ── Always save BOM to Cosmos the moment it exists, regardless of complete flag.
        # This is the authoritative write — the frontend's bomApi.create() is a backup.
        saved_bom_id = None
        if bom_data:
            latest_data_pre = cosmos_client.get_session(request.session_id)
            try:
                pre_session = ChatSession.model_validate(latest_data_pre) if latest_data_pre else session
            except Exception:
                pre_session = session
            saved_bom_id = pre_session.context.bom_id or f"bom_{uuid.uuid4().hex[:12]}"
            pre_session.context.bom_id = saved_bom_id  # persist so regen reuses same ID
            _save_bom_to_cosmos(cosmos_client, bom_data, pre_session, request.user_id or "demo_user")
            logger.info("BOM %s saved to Cosmos from stream (complete=%s)", saved_bom_id, complete)
        ...
        yield "data: " + json.dumps({
            ...
            "progress": progress,
            "bom": bom_data,
            "bom_id": saved_bom_id,   # frontend uses this ID so Redux and Cosmos stay in sync
...
def _save_bom_to_cosmos(cosmos_client, bom_data: dict, session: ChatSession, user_id: str):
    ...
    try:
        ...
        project = (
            ...
            or bom_data.get("project")
            or "New Project"
        )
        category = (
            session.context.category
            or bom_data.get("category")
            or "Data Center / COLO"
        )
        # Preserve the full display name generated by the LLM
        display_name = (
            bom_data.get("name")
            or (f"{project} \u2014 {category} BOM" if project != "New Project" else f"{category} BOM")
        )
        doc = {
            "_id": bom_id,
            "id": bom_id,
            ...
            "category": category,
            "project": project,
            "project_name": project,   # partition key field
            "name": display_name,
            "status": "draft",
        ...
        logger.info("Saved BOM %s ('%s') to Cosmos (session=%s)", bom_id, display_name, session.session_id)
    except Exception as exc:
